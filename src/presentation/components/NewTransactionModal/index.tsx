...
export function NewTransactionModal({ open, onOpenChange }: Props) {
	...
	const selectedCategoryDef: CategoryDefinition | undefined = findCategory(
		currentType,
		currentCategory,
	);
			...
			<Dialog.Portal>
				...
				<Dialog.Content className='fixed left-1/2 top-1/2 z-50 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-dt-border bg-dt-surface p-6 shadow-2xl animate-slide-up mx-4 max-h-[90dvh] overflow-y-auto'>
					...
					<form
						...
						<div>
							<label className='mb-1.5 block text-sm font-medium text-dt-muted'>Tipo</label>
							<Controller
								control={control}
								name='type'
								render={({ field }) => (
						...
						<div>
							<label className='mb-1.5 block text-sm font-medium text-dt-muted'>Descrição</label>
							<div className='flex gap-2'>
								<input
									{...register('description')}
									placeholder='Ex: Aluguel, Plantão 12h...'
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
							<div>
								<label className='mb-1.5 block text-sm font-medium text-dt-muted'>Valor (R$)</label>
								<Controller
									control={control}
									name='amount'
									render={({ field }) => {
								...
								/>
								{errors.amount && <p className='mt-1 text-xs text-dt-red'>{errors.amount.message}</p>}
							</div>

							<div>
								<label className='mb-1.5 block text-sm font-medium text-dt-muted'>Data</label>
						...
						<div>
							...
							{errors.category && <p className='mt-1 text-xs text-dt-red'>{errors.category.message}</p>}
						</div>

						{/* Subcategory — aparece só quando a categoria tem subcategorias */}
						{subcategories.length > 0 && (
							<div className='animate-fade-in'>
								<label className='mb-1.5 block text-sm font-medium text-dt-muted'>Detalhamento</label>
								<div className='relative'>
									<select
						...
						{/* Tags */}
						<div>
							<label className='mb-1.5 block text-sm font-medium text-dt-muted'>Tags (opcional)</label>
							<SmartTagInput
								tags={tags}
								onChange={setTags}
							/>
