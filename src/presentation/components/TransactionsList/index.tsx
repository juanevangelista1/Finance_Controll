		...
		<Dialog.Root
			...
			<Dialog.Portal>
				...
				<Dialog.Content className='fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-dt-border bg-dt-surface p-6 shadow-2xl mx-4 animate-slide-up'>
					<Dialog.Title className='text-lg font-bold text-white mb-2'>Confirmar exclusão</Dialog.Title>
					<p className='text-sm text-dt-muted mb-1'>Deseja remover a transação:</p>
					<p className='text-sm font-semibold text-white mb-6'>&ldquo;{description}&rdquo;</p>
...
function CategoryBadge({
	...
	return (
		<div className='flex flex-col gap-2'>
			<span className='inline-flex items-center gap-1 rounded-full border border-dt-border bg-dt-surface px-1.5 py-1.5 text-sm text-dt-muted sm:max-w-[200px]'>
				<span>{icon}</span>
				{label}
			</span>
			{subLabel && (
		...
		return (
			<div className='space-y-3'>
				{Array.from({ length: 5 }).map((_, i) => (
					<div
						key={i}
						className='h-16 animate-pulse rounded-xl bg-dt-card'
					/>
				))}
			</div>
		);
				...
				{transactions.map((transaction) => (
					<div
						...
						<div className='flex items-start justify-between gap-3'>
							...
							<button
								onClick={() =>
									setDeleteTarget({ id: transaction.id, description: transaction.description })
								}
